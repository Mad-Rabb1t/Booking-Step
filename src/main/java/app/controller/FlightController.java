package app.controller;


import app.io.ConsoleMain;
import app.service.FlightService;
import app.entity.Predicates;
import java.time.LocalDate;

public class FlightController {
    FlightService service;
    ConsoleMain console;

    public void show24H(){
        console.printLn(service.getFlights24H().stream().toString());
    }

    public void showInfo(int flyId){
        console.printLn(service.getFlightBy(Predicates.flightsById(flyId)).toString());
    }

    public void showFlightsByInfo(String dest, String date, int seats){
        console.printLn(service.getFlightBy(Predicates.flightByInfo(dest,LocalDate.parse(date),seats)).toString());
    }


}
